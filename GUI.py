...
layout = [[psg.Text('Введите веса в матрицу смежности', font='Default 12')]] + \
         [[psg.Text(s, key=s, enable_events=True, font='Courier 14', size=(8, 1)) for i, s in enumerate(COL_HEADINGS)]] + \
         ...
         [[psg.Image('pictures/initial.png', key='init_img', visible=False, size=(20, 20))]] + \
         [[psg.Text('Кратчайший гамильтонов цикл', font='Default 12', visible=False, key='t2')]] + \
         [[psg.Image('pictures/0.png', key='i_img', visible=False, size=(20, 20))]] + \
         [[psg.Button('Назад', visible=False, key='back_but'), psg.Button('Далее', visible=False, key='next_but')]]

n = 0
best_paths = []
best_len = 0
max_n = len(best_paths) - 1

window = psg.Window('Метод ближайшего соседа', layout, size=(1000, 850), default_element_size=(12, 1), element_padding=(1, 1), return_keyboard_events=True)
while True:
    ...
    if event == 'Рассчитать':
        ...
        # запускаем алгоритм
        best_len, best_paths = algorithm.closest_neighbor_method(graph)
        max_n = len(best_paths) - 1

        # отображаем результаты
        window['t1'].update(visible=True)
